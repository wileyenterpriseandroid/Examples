...
public class DaoTestClient {
	private String userId = "daoTestUserId";
	...
	public void testUpdate() throws IOException {
		...
		Contact contact = dao.getContact(userId, id);
		assertEquals(firstName, contact.getFirstName());
		assertEquals(1, contact.getVersion());
		contact.setFirstName(updatedFirstName);
		dao.storeOrUpdateContact(userId, contact);
		contact = dao.getContact(userId, id);
	...
	public void testFind() throws IOException {
		...
		int start = 0;
		int numOfmatchers = 2;
		List<Contact> contacts = dao.findContactFirstName(userId, firstName, start, numOfmatchers);
		...
		contacts = dao.findContactFirstName(userId, firstName, start, 10);
	...
	public void testFindChanged() throws IOException {
		...
		List<Contact> list = dao.findChanged(userId, t, 0, 10);		
		assertTrue(list.size() == 5);
		for(Contact cc : list) {
			assertEquals("john123", cc.getFirstName());			
			dao.delete(userId, cc.getId());
		}		
	}	
	...
	public void testgetAll() throws IOException {
		...
		List<Contact> list = dao.getAll(userId, 0, 10);
	...
	public void removeALl() throws IOException {
		for ( Contact c : dao.getAll(userId, 0, 1000)) {
			dao.delete(userId, c.getId());
		}
	}
	...
	public void testFindChangedPaging() throws IOException {
		...
		int start = 1;
		int numOfMatchers = 2;
		List<Contact> list = dao.findChanged(userId, t, start, numOfMatchers);
