...
public class MainActivity extends TabbedActivity {
	...
	// Resource IDs for names of tabs, also used to label data in bundles
	private final int names[] = { R.string.item, R.string.table_info };
	...
	protected void onPause() {
		...
		if (L)
			Log.i(CLASSNAME, "onPause" + (isFinishing() ? " Finishing" : ""));
	}
	...
	protected void onRestoreInstanceState(Bundle savedState) {
		...
		// If we had state to restore, we note that in the log message
		if (L)
			Log.i(CLASSNAME, "onRestoreInstanceState"
					+ (null == savedState ? " Restored state" : ""));
	}
	...
	protected void onUserLeaveHint() {
		...
		if (L)
			Log.i(CLASSNAME, "onUserLeaveHint");
	}
	 ...
	 * 
	 * @param savedState
	 *            the Bundle we got from the callback
	 */
	...
	private void doCreate(Bundle savedState) {
		...
		Class<?> fragments[] = { ItemFragment.class, ItemDetailFragment.class };
	...
	public Bundle buildDataBundle(String item, String dbInfo) {
		...
		data.putString(getDataLabel(0), item);
		data.putString(getDataLabel(1), dbInfo);
		return data;

	}

	public String getDataLabel(int position) {
		return getResources().getString(names[position]);
	}
